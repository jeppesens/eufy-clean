...
class SharedConnect(Base):
    def __init__(self, config) -> None:
        ...
        self.device_model_desc = (
            EUFY_CLEAN_DEVICES.get(self.device_model, "") or self.device_model
        )
        self.config = {}
    ...
    async def _map_data(self, dps):
        ...
        try:
            ...
            for k, v in self.robovac_data.items():
                if isinstance(v, (bytes, bytearray)):
                    _LOGGER.debug(
                        "robovac_data[%s] type=%s len=%d hex=%s",
                        k,
                        type(v).__name__,
                        len(v),
                        v.hex(),
                    ...
                    try:
                        ...
                        hexed = repr(v)
                    _LOGGER.debug(
                        "robovac_data[%s] type=str repr=%r hex=%s", k, v, hexed
                    )
                else:
                    _LOGGER.debug(
                        "robovac_data[%s] type=%s repr=%r", k, type(v).__name__, v
                    )
        except Exception:
        ...
        for listener in self._update_listeners:
            try:
                _LOGGER.debug(
                    "Calling listener %s", getattr(listener, "__name__", repr(listener))
                )
    ...
    async def get_clean_speed(self):
        ...
        try:
            ...
            if isinstance(clean_speed_raw, list) and len(clean_speed_raw) > 0:
                speed = int(
                    clean_speed_raw[0]
                )  # Fixed: use [0] instead of treating list as int
    ...
    async def get_control_response(self) -> ModeCtrlResponse | None:
        try:
            ...
            if raw is None:
                _LOGGER.debug(
                    "get_control_response: PLAY_PAUSE missing from robovac_data"
                )
    ...
    async def get_work_mode(self) -> str:
        try:
            ...
            if not mode:
                ...
                )  # Fixed: actually return the mode
        except Exception:
            return "auto"
            ...
            match value.state:
                ...
                case _:
                    ...
                    if hasattr(value, "State") and hasattr(value.State, "DESCRIPTOR"):
                        state_val = value.State.DESCRIPTOR.values_by_number.get(
                            value.state
                        )
                        if state_val:
    ...
    async def get_clean_params_request(self):
        try:
            value = decode(
                CleanParamRequest, self.robovac_data.get("CLEANING_PARAMETERS")
            )
            return value
        except Exception as e:
    ...
    async def get_clean_params_response(self):
        try:
            value = decode(
                CleanParamResponse, self.robovac_data.get("CLEANING_PARAMETERS")
            )
            return value or {}
        except Exception:
            return {}
    ...
    async def get_scene_list(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            _LOGGER.debug(
                f"Attempting to decode scene data (length: {len(scene_data)})"
            )
            ...
            for scene_info in scene_response.infos:
                ...
                if scene_info.name and scene_info.valid:
                    scenes.append(
                        {
                            "id": (
                                scene_info.id.value if scene_info.HasField("id") else 0
                            ),
    ...
    async def get_map_info(self) -> dict[str, Any] | None:
        ...
        try:
            ...
            _LOGGER.debug(
                f"Attempting to decode map data from DPS 165 (length: {len(map_data)})"
            )

            # UniversalDataResponse check (UniversalDataResponse has RoomTable cur_map_room)
            try:
                universal_data = decode(
                    UniversalDataResponse, map_data, has_length=True
                )
                if universal_data and universal_data.cur_map_room.map_id:
                    ...
                    _LOGGER.debug(
                        "Successfully decoded DPS 165 as UniversalDataResponse: map_id=%d, rooms=%r",
                        universal_data.cur_map_room.map_id,
                        rooms,
                    )
                    self.map_id = universal_data.cur_map_room.map_id
                    self.rooms = rooms
                    return {"map_id": self.map_id, "rooms": self.rooms}
            except Exception:
            ...
            try:
                ...
                if room_params and room_params.map_id:
                    ...
                    _LOGGER.debug(
                        "Successfully decoded DPS 165 as RoomParams: map_id=%d, releases=%d, rooms=%r",
                        room_params.map_id,
                        room_params.releases,
                        rooms,
                    )
    ...
    async def set_clean_speed(self, clean_speed: EUFY_CLEAN_CLEAN_SPEED):
        try:
            set_clean_speed = [s.lower() for s in EUFY_CLEAN_NOVEL_CLEAN_SPEED].index(
                clean_speed.lower()
            )
            _LOGGER.debug(
                "Setting clean speed to: %r %r %r",
                set_clean_speed,
                EUFY_CLEAN_NOVEL_CLEAN_SPEED,
                clean_speed,
            )
    ...
    async def scene_clean(self, id: int):
        ...
        value = encode(
            ModeCtrlRequest,
            {
                ...
                "scene_clean": {"scene_id": id + increment},
            },
        )
        return await self.send_command({self.dps_map["PLAY_PAUSE"]: value})
    ...
    async def room_clean(self, room_ids: list[int], map_id: int = 3):
        ...
        rooms_clean = SelectRoomsClean(
            rooms=[
                SelectRoomsClean.Room(id=id, order=i + 1)
    ...
    async def set_clean_param(self, config: dict[str, Any]):
        ...
        if ct := config.get("clean_type"):
            if ct not in CleanType.Value.keys():
                raise ValueError(
                    f"Invalid clean type: {ct}, allowed values: {CleanType.Value.keys()}"
                )
        ...
        if ce := config.get("clean_extent"):
            if ce not in CleanExtent.Value.keys():
                raise ValueError(
                    f"Invalid clean extent: {ce}, allowed values: {CleanExtent.keys()}"
                )
            clean_extent = {
                "value": CleanExtent.Value.DESCRIPTOR.values_by_name[ce].number
            }
        else:
            clean_extent = {}
        ...
        if is_mop and (mm := config.get("mop_mode")):
            if mm not in MopMode.Level.keys():
                raise ValueError(
                    f"Invalid mop mode: {mm}, allowed values: {MopMode.Level.keys()}"
                )
